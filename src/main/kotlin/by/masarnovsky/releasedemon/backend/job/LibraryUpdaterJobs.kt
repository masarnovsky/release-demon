...
class LibraryUpdaterJobs(
  ...
  fun updateLibraryFromLastfm(chatId: Long) {
    ...
    userService.findByTelegramId(chatId)?.let { user -> updateUserLibraryFromLastfm(user) }
  }
  ...
  private fun mergeUserArtists(user: User, actualUserArtists: List<String>): User {
    logger.info {
      "user artists in db: ${user.artists.size}, retrieved from remote library: ${actualUserArtists.size}"
    }
  ...
  private fun saveNonExistingArtistsFromUserLibrary(
    ...
    val newArtists =
        actualUserArtists.filter { artistName -> !artistNames.contains(artistName) }.map {
          Artist(name = it)
        }
